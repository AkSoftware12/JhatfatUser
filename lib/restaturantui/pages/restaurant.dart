...
class _RestaurantState extends State<Restaurant_Sub> {
  ...
  @override
  void initState() {
    print("Page of Restaurant");
    getdata();
    hitResturantItem();
    getCartCount();
    getCartItem();
    print("List image is : $listImage");
    super.initState();
  }
  ...
  void hitResturantItem() async {
    print('vendor id of Restaurant is:${widget.item.vendor_id}');
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonData['status'] == "1") {
          ...
          if (tagObjs != null && tagObjs.length > 0) {
            ...
            print("category List of Restaurant : ${categoryList}");
            print("category List 2 : ${categoryList2}");
          } else {}
        }
      } else {}
    });
  ...
  void hitSliderUrl() async {
    ...
    http.get(myUri).then((response) {
      if (response.statusCode == 200) {
        var jsonData = jsonDecode(response.body);
        print('Response Restauarnt Body: - ${response.body}');
        ...
        if (jsonData['status'] == "1") {
          ...
          if (tagObjs != null && tagObjs.isNotEmpty) {
            ...
            print("list Image2 is: $listImage");
          } else {
            setState(() {
              isSlideFetch = false;
            });
                        ...
                        Visibility(
                          ...
                          child: Container(
                            ...
                            child: Padding(
                              ...
                              child: (listImage != null && listImage.isNotEmpty)
                                  ? ListView.builder(
                          ...
                          child: TypeAheadField(
                            ...
                            onSuggestionSelected: (Vendors detail) {
                              for (int i = 0; i < categoryList.length; i++) {
                                if (detail.product_id.toString() ==
                                        categoryList[i].product_id.toString() ||
                                    detail.str1
                                            .toString()
                                            .trim()
                                            .toLowerCase() ==
                                  ...
                                  print(
                                      "CLICKED: " + categoryList[i].toString());
                                }
                              }
                            },
                          ),
  ...
  void CallSearch(CategoryResturant categoryListNew, int index) {
    {
      ...
      DatabaseHelper db = DatabaseHelper.instance;
      db
          .getRestProdQty('${categoryListNew.variant[0].variant_id}')
          .then((value) {
        if (value != null) {
          setState(() {
        ...
        db
            .calculateTotalRestAdonA('${categoryListNew.variant[0].variant_id}')
            .then((value1) {
          double priced = 0.0;
          if (value != null) {
          ...
          db
              .getAddOnList('${categoryListNew.variant[0].variant_id}')
              .then((valued) {
            List<AddonList> addOnlist = [];
            if (valued != null && valued.length > 0) {
              addOnlist = valued.map((e) => AddonList.fromJson(e)).toList();
              for (int i = 0; i < categoryListNew.addons.length; i++) {
                int ind = addOnlist.indexOf(
                    AddonList('${categoryListNew.addons[i].addon_id}'));
                if (ind != null && ind >= 0) {
                  setState(() {
                    categoryListNew.addons[i].isAdd = true;
                  });
                }
              }
            }
