...
class _ProductTabDataState extends State<ProductTabData> {
  ...
  void hitResturantItem() async {
    ...
    print('dishes vendor id is: ${widget.item.vendor_id}');
  ...
  void hitSliderUrl() async {
    ...
    print("hit slide vendor id : ${widget.item.vendor_id}");
    setState(() {
      isSlideFetch = true;
    });
    ...
    http.post(myUri, body: {
      ...
      if (response.statusCode == 200) {
        var jsonData = jsonDecode(response.body);
        print('Response Body of hit side: - ${response.body}');
        if (jsonData['status'] == "1") {
          var tagObjsJson = jsonDecode(response.body)['data'] as List;
